...
export function ReviewActions({ propertyId }: { propertyId: string }) {
  ...
  const submit = async (decision: 'verified' | 'rejected') => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/admin/review-property', {
        method: 'POST',
      ...
      <CardContent>
        ...
        {mode === 'verify' && (
          <div className="space-y-3">
            <p className="text-sm">
              Bestätigst du, dass die Eigentumsnachweise gültig sind und du die Immobilie
              freigeben möchtest?
            </p>
            {error && <p className="text-sm text-destructive">{error}</p>}
            <div className="flex gap-2">
              <Button onClick={() => submit('verified')} disabled={submitting}>
                {submitting ? 'Wird gespeichert…' : 'Ja, freigeben'}
              </Button>
        ...
        {mode === 'reject' && (
          <div className="space-y-3">
            <div className="space-y-2">
              <Label htmlFor="reason">Begründung (wird dem Antragsteller angezeigt)</Label>
              <Textarea
                id="reason"
                rows={3}
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="z.B. Grundbuchauszug nicht aktuell"
              />
            </div>
            {error && <p className="text-sm text-destructive">{error}</p>}
            <div className="flex gap-2">
